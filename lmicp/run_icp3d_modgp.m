function [params,scale,t,mean_err] = run_icp3d_modgrb(Model, Data, ROBUST_THRESH, initial_p, PLOT_ON)
...
% Date: 31 Aug 01
% Modified GRB Oct 2005
% Modified GP Feb 2010
%% added output arguments scale,t,mean_err
%% also allowed variable number, or empty inputs

global run_icp3d_iter do_plot
...
disp('No mean correction for model and data points (in run_icp3d_modgrb)');
ModelCenter=[0 0 0];
...
%% t= [46.66865139483727  20+32.96533175806930  20+35.93200737885691]
...
%hm=scatter(BOX * (N+1), 'b-')
hm=scatter3(BOX(:,1) * (N+1), BOX(:,2) * (N+1), BOX(:,3) * (N+1),'w')
hold on
scatter3(Model(:,2),Model(:,1),Model(:,3),'r.');
%set(scatter(Model(:,[2 1 3]), 'r.'), 'markersize', 0.5)
camlight
hold on
%h = scatter(Data(:,[2 1 3]), '.');
h = scatter3 (Data(:,2),Data(:,1),Data(:,3),'b.');
if size(Data,1) > 1000
  disp('small markers');
...
if DO_LM
  ...
  if ~isempty(initial_p),
    disp('using given initial params');
    params = initial_p;
  end
  ...
  [params, resnorm, residual, exitflag, output] =...
   ...
   [dists, J] = icp_error_with_derivs(params, icp);
