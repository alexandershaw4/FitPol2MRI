...
v=zeros(N,dim);
d=zeros(N,1);
NewPx(1,:)=NewP(1,:)+Cref(1);				% Mod dec98
NewPx(2,:)=NewP(2,:)+Cref(2);				% Mod dec98	
NewPx(3,:)=NewP(3,:)+Cref(3);				% Mod dec98	
v=finddist2(NewPx,width,height,depth,refno,dmloc);		
d=sqrt((v.*v)*[1;1;1]);	
err(1,1)=mean(d);
err(1,2)=std(d);
err(1,3)=median(d);
